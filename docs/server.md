...
| `limits.retention_hours` | upper bound on the age of an observation (24). Observations live in their session and are freed with it, a minute after it expired (`session_ttl_seconds`, at most 600), so this bound only acts when it is shorter than a session's life |
| `limits.max_observations` | ceiling on observations held across all sessions (100000). At the ceiling new observations are dropped until sessions expire; stored ones are never evicted. One session holds at most 8192 (built in; a full 3-round scan leaves about 2000) |
...
reservation. Observations are kept per session: a flow the server cannot
attribute to a live session (no reservation, an unknown or expired session id
in an envelope, DNS name or echo path) is not recorded, so strangers cannot
evict real evidence. Dropped rows are counted and reported once a minute at
`log.level: debug`.
...
Without a reservation or cookie the server **stays silent**. Such a datagram
belongs to no session, and observations are only served to their session, so
it is not recorded either (it never was readable: the observations endpoint
returns a session's own rows). `unmatched: true` remains on rows that do name
a session but matched no reservation.
...
the in-memory session for correlation and is not exported. Observations are
kept in memory inside their session and freed with it (bounded by
`max_observations` overall and `retention_hours` in age); a restart clears
them.
