...
| `--adaptive-timeout` | true | per-attempt timeout = 6 × control RTT + 0.5 s, clamped to 1.5 s … `--timeout`. The `dest.*` exchanges past the TCP connect (TLS handshake, DoH and system lookups, the disagreement check, HTTP) get at least 4 s within `--timeout`: a real site's handshake is several round trips with a certificate flight, not the one small exchange the control RTT measures |
...
server was unreachable (`probe_unreachable`) or an I/O error. With `--sites`
an unreachable server does not cost the site results: the real-destinations
family needs no server, so it runs and is reported under the
`PROBE UNREACHABLE` line (the exit code stays 1).
...
`http` (add the application-layer check), `proto=whatsapp` (the address
speaks that protocol, not web TLS: its own handshake, `dest.proto`, replaces
the TLS layers). The built-in catalog is in
...
(including the Telegram data-centre addresses, TCP only, and the WhatsApp
chat gateway with its own handshake), the app stores,
...
| `dest.proto` | tcp/443 | `<domain>` | for `proto=` targets, in place of `dest.tls`: the protocol's own opening exchange on the same address. `whatsapp`: the WhatsApp clients' Noise prologue (`WA`, magic 6, dictionary 3) and a ClientHello frame with a random ephemeral key; a server hello with a 32-byte ephemeral key, a static key and a payload passes (`detail.server_hello` has their lengths). A reply that is not one is `unexpected_response`; a reset keeps `detail.reset_ms` like `dest.tls`. A TLS ClientHello to this server is reset by the server itself on every network, which is why such a target has no SNI differential |
