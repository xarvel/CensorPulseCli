...
type Target struct {
	...
	// Proto is the protocol the address speaks instead of web TLS (Protos):
	// its own opening exchange (dest.proto) replaces the TLS handshakes.
...
func Default() []Target {
	return []Target{
		...
		{Domain: "g.whatsapp.net", Category: CategoryMessenger, Label: "WhatsApp backend", Proto: ProtoWhatsApp},
...
//
//	category=news  label="..."  ip=1.2.3.4,5.6.7.8  sni=name  dns=name  tcp-only  no-dns  http  proto=whatsapp
//
// Unknown categories are kept as "custom"; a line without a domain, or an
// unknown proto=, is an error.
func Parse(r io.Reader) ([]Target, error) {
	...
	for sc.Scan() {
		...
		if t.Domain == "" || strings.ContainsAny(t.Domain, "=/ ") {
		...
		for _, f := range fields[1:] {
			...
			case "proto":
				if !knownProto(v) {
					return nil, fmt.Errorf("line %d: proto=%s: unknown protocol (known: %s)", line, v, strings.Join(Protos, ", "))
				}
				t.Proto = v
			default:
				if hasValue {
...
func Compare(sys SystemResult, trusted Consensus) Comparison {
	...
	case sysPublic > 0 && trustedPublic > 0 && !sharesFamily(sys.IPs, trusted.IPs):
		// An A answer and an AAAA answer never share an address: a trusted
		// view that lost its A query to a timeout (or a platform resolver
		// that asks for A only on an IPv4 network) is not a disagreement.
		return Comparison{Note: "the system and trusted answers are of different address families (A records on one side, AAAA on the other): not compared"}
...
// sharesFamily reports whether both lists hold a public address of one
// family.
func sharesFamily(a, b []netip.Addr) bool {
	fam := func(ips []netip.Addr) (v4, v6 bool) {
		for _, ip := range ips {
			if IsBogon(ip) {
				continue
			}
...
// Usable keeps the addresses usable reports the host can connect to; a nil
// usable keeps them all.
func Usable(ips []netip.Addr, usable func(netip.Addr) bool) []netip.Addr {
	if usable == nil {
		return ips
	}
	var out []netip.Addr
...
// ProbeAddrs picks what the transport layers probe: the trusted answers the
// host can connect to, else the system's public ones (every DoH resolver
// blocked, or a trusted answer only of a family the host has no route for,
// such as AAAA on an IPv4-only phone), and says which. usable filters the
// addresses by what the host can reach; nil keeps them all.
func ProbeAddrs(sys SystemResult, trusted Consensus, usable func(netip.Addr) bool) (ips []netip.Addr, source string) {
	if t := Usable(Prioritize(trusted.IPs), usable); len(t) > 0 {
		return t, "trusted"
	}
	if s := Usable(Prioritize(sys.IPs), usable); len(s) > 0 {
		return s, "system"
	}
	return nil, "none"
}
