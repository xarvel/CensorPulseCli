...
func TestCompareRules(t *testing.T) {
	cases := []struct {
		...
		// A field run: the trusted A queries timed out, only an AAAA came back,
		// the system resolver answered A records. Nothing to compare.
		{"families apart", SystemResult{IPs: []netip.Addr{ip("108.177.14.93"), ip("108.177.14.91")}, RCode: "NOERROR"}, Consensus{IPs: []netip.Addr{ip("2a00:1450:400f:810::200e")}, RCode: "NOERROR"}, Comparison{}},
		// One family in common is compared on it.
		{"one family shared", SystemResult{IPs: []netip.Addr{ip("5.5.5.5")}, RCode: "NOERROR"}, Consensus{IPs: []netip.Addr{ip("1.1.1.1"), ip("2606:4700::1111")}, RCode: "NOERROR"}, Comparison{Disagree: true}},
	}
	for _, c := range cases {
...
func TestProbeAddrsFallsBackToPublicSystemAnswers(t *testing.T) {
	ips, src := ProbeAddrs(SystemResult{IPs: []netip.Addr{ip("127.0.0.1"), ip("5.5.5.5")}}, Consensus{RCode: "ERROR"}, nil)
	if src != "system" || Strings(ips) != "5.5.5.5" {
		t.Fatalf("got %s %s", src, Strings(ips))
	}
	if _, src := ProbeAddrs(SystemResult{}, Consensus{}, nil); src != "none" {
		t.Fatalf("got %s", src)
	}
}

// A field run: the trusted view of youtube.com was one AAAA record, the
// phone had no IPv6 route, and the site read as blackholed. Without a route
// the v6 answer is not probed; the system resolver's v4 answers are.
func TestProbeAddrsSkipsAFamilyTheHostCannotReach(t *testing.T) {
	v4only := func(a netip.Addr) bool { return a.Unmap().Is4() }
	sys := SystemResult{IPs: []netip.Addr{ip("108.177.14.93"), ip("108.177.14.91")}, RCode: "NOERROR"}
	trusted := Consensus{IPs: []netip.Addr{ip("2a00:1450:400f:810::200e")}, RCode: "NOERROR"}
	ips, src := ProbeAddrs(sys, trusted, v4only)
	if src != "system" || Strings(ips) != "108.177.14.93,108.177.14.91" {
		t.Fatalf("no IPv6 route: got %s %s", src, Strings(ips))
	}
	// With a route the trusted answer is probed, v6 or not.
	if ips, src := ProbeAddrs(sys, trusted, nil); src != "trusted" || Strings(ips) != "2a00:1450:400f:810::200e" {
		t.Fatalf("with IPv6: got %s %s", src, Strings(ips))
	}
	// A mixed trusted answer keeps its v4 part; nothing usable at all is "none".
	trusted.IPs = append(trusted.IPs, ip("142.250.1.1"))
	if ips, src := ProbeAddrs(sys, trusted, v4only); src != "trusted" || Strings(ips) != "142.250.1.1" {
		t.Fatalf("mixed: got %s %s", src, Strings(ips))
	}
	if _, src := ProbeAddrs(SystemResult{IPs: []netip.Addr{ip("2a00::1")}}, Consensus{IPs: []netip.Addr{ip("2a00::2")}}, v4only); src != "none" {
		t.Fatalf("v6 only on both sides: got %s", src)
	}
}
...
func TestQueryDoHWireAndFallbacks(t *testing.T) {
	...
	// QueryDoH fans A+AAAA out concurrently, so the handler is hit in parallel.
	...
	if posts.Load() == 0 || gets.Load() == 0 {
		t.Fatalf("expected POST then GET, got %d/%d", posts.Load(), gets.Load())
	}
