...
// The fault proxy listens on a second loopback address and relays to the
// server on 127.0.0.1, so the client is pointed at the proxy and sees the
// injected path behaviour while the server's observations stay honest.
// Linux answers on all of 127.0.0.0/8; macOS has no 127.0.0.2 without
// `sudo ifconfig lo0 alias 127.0.0.2 up`, but the server binds 127.0.0.1
// only, so the same ports are free on ::1.
var proxyIP = pickProxyIP()

func pickProxyIP() string {
	...
	return "127.0.0.2" // startProxy fails with the bind error
}
