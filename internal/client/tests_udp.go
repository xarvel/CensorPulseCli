...
func (c *Client) udpConn(a *Attempt) (*net.UDPConn, error) {
	...
	if err != nil {
		return nil, dialError(err, 0, true)
	}
...
// udpOutcome maps a UDP exchange error. connect_refused here is an ICMP port
// unreachable surfaced by the kernel.
func udpOutcome(err error) string { return classifyNetErr(err, true) }
