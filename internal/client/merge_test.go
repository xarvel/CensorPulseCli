...
// The cut position of tcp.packets comes from the attached observation, so a
// first merge (not only a re-merge of a report that already carries its
// server half) has to produce it.
func TestMergeTCPPacketsSeenOnFirstMerge(t *testing.T) {
	a := &Attempt{TestID: "tcp.packets", Transport: "tcp", DstPort: 443, SrcPort: 50001, Outcome: OutcomePayloadTimeout,
		Detail: map[string]string{"chunk_bytes": "2"}, Stages: map[string]float64{}}
	o := model.Observation{TestID: "tcp.packets", Transport: "tcp", DstPort: 443, SrcPort: 50001,
		Detail: map[string]string{"partial": "true", "partial_bytes": "13"}}
	mergeObservations([]*Attempt{a}, []model.Observation{o}, 0)
	if a.Server == nil {
		t.Fatal("observation not attached by 5-tuple")
	}
	if got := a.Detail["server_packets_seen"]; got != "7" {
		t.Fatalf("server_packets_seen = %q, want 7 (13 bytes in 2-byte segments)", got)
	}
}
