...
func New(opt Options) (*Client, error) {
	...
	c.http = &http.Client{Timeout: 15 * time.Second, Transport: &http.Transport{
		...
		TLSClientConfig:   c.pinnedTLSConfig("", true),
...
// pinnedTLS returns a client config that ignores names and CAs and, when a
// pin is known, enforces it. The test flows use it: what they are presented
// with is evidence of the path, not the control point's identity, so they
// never record it.
func (c *Client) pinnedTLS(serverName string) *tls.Config {
	return c.pinnedTLSConfig(serverName, false)
}

// pinnedTLSConfig is pinnedTLS that, with record set, also keeps the pin it
// saw in ObservedPin / PinMatched: the control transport's config.
func (c *Client) pinnedTLSConfig(serverName string, record bool) *tls.Config {
	return &tls.Config{
		...
		VerifyPeerCertificate: func(raw [][]byte, _ [][]*x509.Certificate) error {
			...
			matched := want == "" || want == pin
			if record {
				// Control connections can be re-dialled from several
				// workers at once.
				c.pinMu.Lock()
				c.ObservedPin = pin
				c.PinMatched = want != "" && matched
				c.pinMu.Unlock()
			}
			if !matched {
