...
// A resolver that agrees with the trusted ones on no site (dead, captive, or
// a platform resolver reporting its own failure as "no such host") is one
// finding about the resolver: no site is called poisoned on it.
func TestDestSystemResolverUnreliableIsOneVerdict(t *testing.T) {
	at := destControls(2, "ok", "ok")
	for _, d := range []string{"a.example", "b.example", "c.example"} {
		at = append(at, destSite(d, "news", 2, "dns_answer_mismatch", "ok", "ok", "ok", "ok")...)
	}
	r := ClassifyStandalone(at)
	k := kinds(r)
	if k["system_resolver_unreliable_suspected"] != 1 || k["dest_dns_poisoning_suspected"] != 0 {
		t.Fatalf("verdicts: %+v", r.Verdicts)
	}
	for _, d := range r.Destinations {
		if d.Verdict == "dest_dns_poisoning_suspected" {
			t.Fatalf("%s called poisoned without a control: %+v", d.Domain, d)
		}
	}
	// One mismatching site among sites that resolve is still that site's finding.
	at = append(at, destSite("wikipedia.org", "control", 2, "ok", "ok", "ok", "ok", "ok")...)
	k = kinds(ClassifyStandalone(at))
	if k["system_resolver_unreliable_suspected"] != 0 || k["dest_dns_poisoning_suspected"] != 3 {
		t.Fatalf("with a passing control: %+v", k)
	}
}
