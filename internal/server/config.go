...
type Config struct {
	...
	Limits struct {
		...
		MaxObservations    int `yaml:"max_observations"` // ceiling across all sessions
...
func (c *Config) Validate() error {
	...
	// An explicit 0 here is not "default": a zero ceiling stores no
	// observation at all and a zero idle timeout expires every flow before
	// its first byte.
	if c.Limits.MaxObservations <= 0 {
		return fmt.Errorf("config: max_observations must be at least 1")
	}
	if c.Limits.IdleTimeoutSeconds <= 0 {
		return fmt.Errorf("config: idle_timeout_seconds must be at least 1")
	}
	if c.DNSZone == "" {
		c.DNSZone = "probe.invalid"
	}
