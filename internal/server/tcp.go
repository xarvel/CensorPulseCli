...
func (s *Server) handleTCP(c net.Conn, port int, role tcpRole) {
	...
	pc := &peekConn{Conn: c}
	idle := time.Duration(s.cfg.Limits.IdleTimeoutSeconds) * time.Second
...
func (s *Server) finish(obs *model.Observation, pc *peekConn, ip netip.Addr, kind string) {
	...
	// Flows nobody can claim are kept only for addresses with a live
	// session (NAT diagnostics); a stranger's connections must not evict the
	// evidence of real scans from the ring.
	if obs.Unmatched && obs.SessionID == "" && !s.store.ipHasSession(ip) {
		return
	}
	s.store.addObservation(*obs)
	if s.cfg.Log.Level == "debug" {
		s.log.Debug("tcp flow", "port", obs.DstPort, "parse", obs.Parse, "response", obs.Response, "close", obs.Close, "in", obs.BytesIn, "out", obs.BytesOut, "session", obs.SessionID != "")
	}
}
...
func (s *Server) tcpTLS(pc *peekConn, obs *model.Observation, ip netip.Addr) {
	...
	first, err := inner.sniff(1, time.Duration(s.cfg.Limits.IdleTimeoutSeconds)*time.Second)
	if len(first) == 0 {
		obs.Close = closeReason(err)
		return
	}
	...
	case vless.IsRequest(first):
		...
		obs.RepliedAt = ptrTime(time.Now())
		sub := model.Observation{Detail: map[string]string{}}
		s.tcpEchoStream(inner, &sub, time.Duration(s.cfg.Limits.IdleTimeoutSeconds)*time.Second)
...
func (s *Server) tcpDNS(pc *peekConn, obs *model.Observation, viaTLS bool) {
	pc.SetReadDeadline(time.Now().Add(time.Duration(s.cfg.Limits.IdleTimeoutSeconds) * time.Second))
