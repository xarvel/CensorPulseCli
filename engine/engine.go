...
// complete. Done and Total count attempts, including retry rounds, which
// enter Total as soon as a failure foretells them (client.Run), not when
// they start: Done grows by one with every delivery and Total never shrinks.
...
// sitesWithoutProbe plans the real-destinations family for a scan whose
// control point did not answer. The family needs no server, and a network
// that blocks the control point's address is exactly where its results are
// wanted: the report stays "probe unreachable" and carries the sites anyway.
func (s *Scan) sitesWithoutProbe(opt client.Options) {
	var tests []string
	for _, t := range s.cfg.Tests {
		if client.IsDestTest(t) {
			tests = append(tests, t)
		}
	}
	if !s.cfg.Sites && len(tests) == 0 || len(s.cfg.Tests) > 0 && len(tests) == 0 {
		return // the family was not asked for
	}
	c, err := client.New(client.Options{Sites: true, Targets: opt.Targets, Tests: tests, Repeat: opt.Repeat, Retry: opt.Retry,
		Parallel: opt.Parallel, Timeout: opt.Timeout, LocalAddr: opt.LocalAddr, Log: opt.Log, SystemResolver: opt.SystemResolver})
	if err != nil {
		return
	}
	s.c, s.plans = c, c.Catalog()
	s.rep.TimeoutMs = float64(c.EffectiveTimeout().Milliseconds())
}

// ClassifyAttempts picks the classifier for a report's attempts: the
// standalone family for a sites scan, the full rule set for a scan that
// reached its control point. A scan that did not reach it has no
// protocol-level conclusion, but the real-destinations family it ran without
// the server keeps its cells, verdicts and per-site summary.
func ClassifyAttempts(attempts []*client.Attempt, sites, probeReached bool) classify.Result {
	switch {
	case sites:
		return classify.ClassifyStandalone(attempts)
	case probeReached:
		return classify.Classify(attempts, true)
	}
	res := classify.Classify(nil, false)
	if len(attempts) > 0 {
		d := classify.ClassifyStandalone(attempts)
		res.Cells, res.Destinations, res.Notes = d.Cells, d.Destinations, d.Notes
		res.Verdicts = append(res.Verdicts, d.Verdicts...)
		res.Summary += "; sites: " + d.Summary
	}
	return res
}
...
func (s *Scan) Run(ctx context.Context) (*report.Report, error) {
	...
	s.ran = true
	rep := s.rep
	if !s.reachable && len(s.plans) == 0 {
		res := ClassifyAttempts(nil, false, false)
	...
	if (rep.Sites() || !s.reachable) && s.cfg.GeoIPOnline {
		s.onlineNetwork(ctx)
	}
	...
	if ctx.Err() != nil {
		...
		// The caller's cancel and the scan's own deadline both end up in
		// ctx; only the second one is the deadline.
		if parent.Err() != nil {
			rep.Notes = append(rep.Notes, fmt.Sprintf("the run was cancelled; %s not reached are missing, not clear", what))
		} else {
	...
	if s.reachable && !rep.Sites() {
		rep.Notes = append(rep.Notes, BurstNotes(attempts)...)
	}
	res := ClassifyAttempts(attempts, rep.Sites(), s.reachable)
	...
	if !s.reachable {
		rep.Notes = append(rep.Notes, "the control point did not answer: only the real-destinations family ran, which needs no server")
		rep.FinishedAt = time.Now()
		return rep, ErrProbeUnreachable
	}
