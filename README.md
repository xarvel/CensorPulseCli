...
| Real destinations | `dest.dns`, `dest.nxdomain`, `dest.tcp`, `dest.tls`, `dest.http` (`--sites`, or `cpprobe sites` without a server) | how the network treats the sites people open: system resolver against three DoH resolvers, TCP to the site's address, the SNI differential (real name, decoy name, no name on the same address), HTTP 451; one-sided evidence, verdicts capped at medium |
