...
func runSites(args []string) error {
	...
	fs.Usage = func() {
		...
		fmt.Fprintln(os.Stderr, "  category=news  label=\"...\"  ip=1.2.3.4,5.6.7.8  sni=name  dns=name  tcp-only  no-dns  http  proto=whatsapp")
		fmt.Fprintln(os.Stderr, "\nFlags:")
		fs.PrintDefaults()
	}
...
func runClassify(args []string) error {
	...
	res := engine.ClassifyAttempts(rep.Attempts, rep.Sites(), rep.ProbeReached)
...
func runScan(args []string) error {
	...
	if !s.Reachable() {
		if *planOnly {
			// --plan neither runs anything nor writes a report.
			return fmt.Errorf("cannot plan: %s: %s", engine.ErrProbeUnreachable, s.Report().BootstrapErr)
		}
		rep, _ := s.Run(ctx)
		return finish(rep, *out, *jsonOnly, style)
	}
...
// parseFlags parses args and refuses leftovers. flag stops at the first
// positional argument, so every flag after a stray word (a --pin, say) would
// be silently ignored and the command would run without it.
func parseFlags(fs *flag.FlagSet, args []string) error {
	fs.Parse(args)
	if fs.NArg() > 0 {
		return fmt.Errorf("%s: unexpected argument %q (no command takes positional arguments; the flags after it were not read)", fs.Name(), fs.Arg(0))
	}
	return nil
}
