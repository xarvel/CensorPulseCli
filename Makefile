...
# 15+: every PT_LOAD segment of libgojni.so aligned to 16384. NDKs before r28
# link 4 KB segments, so the alignment is passed to the linker explicitly
# (harmless on r28+, where it is the default); scripts/check-16k.sh verifies
# the AAR, and bind.yml runs it before publishing.
ANDROID_LDFLAGS := -extldflags=-Wl,-z,max-page-size=16384

bind-android: bind-tools
	@mkdir -p dist
	$(GOMOBILE) bind -target android/arm64,android/arm,android/amd64 -androidapi 24 -o dist/cpprobe.aar -ldflags="$(BIND_LDFLAGS) $(ANDROID_LDFLAGS)" ./bind
