...
```go
OnProgress(done, total int, testID, group, variant, outcome string)  // after each attempt; total grows with retry rounds
...
`probe_unreachable`) and no error, because the generated Java and
