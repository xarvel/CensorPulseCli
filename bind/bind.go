...
type Listener interface {
	...
	// far (total grows while a round runs, as each failure foretells the
	// retry rounds of its port group; it never shrinks), the test id, its
	// classification group ("tcp/443", "dest/<domain>"), the variant and
	// the client-side outcome ("ok", "connect_timeout", …; docs/report.md).
...
// guard turns a panic on the calling goroutine into an error. Under gomobile
// a Go panic takes the whole app down, so nothing may cross the binding.
func guard(err *error) {
	if r := recover(); r != nil {
		*err = fmt.Errorf("cpprobe: internal error: %v", r)
	}
}

// quiet runs a callback into the embedder's code. The callbacks run on the
// engine's worker goroutines, out of reach of Run's guard, and an exception
// thrown on the other side of the binding arrives here as a panic: a lost
// progress or log line is the better outcome.
func quiet(f func()) {
...
// system resolver).
func NewScan(configJSON string, listener Listener, resolver Resolver) (_ *Scan, err error) {
	defer guard(&err)
	c, err := parseConfig(configJSON)
	if err != nil {
		return nil, err
...
// called once.
func (s *Scan) Run() (_ string, err error) {
	defer guard(&err)
	s.mu.Lock()
	if s.started {
		s.mu.Unlock()
	...
	if s.listener != nil {
		...
		hooks.Progress = func(p engine.Progress) {
			quiet(func() { l.OnProgress(p.Done, p.Total, p.TestID, p.Group, p.Variant, p.Outcome) })
		}
		hooks.Log = slog.New(slog.NewTextHandler(&logWriter{l: l}, &slog.HandlerOptions{Level: slog.LevelInfo}))
		hooks.Notice = func(msg string) { quiet(func() { l.OnLog("info", msg) }) }
	}
	if s.resolver != nil {
		r := s.resolver
...
func lookupVia(ctx context.Context, r Resolver, host string) ([]netip.Addr, error) {
	...
	go func() {
		...
		defer func() {
			if p := recover(); p != nil {
				res = result{err: fmt.Errorf("resolver: %v", p)}
			}
			ch <- res
		}()
		res.s, res.err = r.Lookup(host)
	}()
	var res result
	select {
	case res = <-ch:
...
func (w *logWriter) Write(p []byte) (int, error) {
	...
	for {
		...
		quiet(func() { w.l.OnLog(level, line) })
	}
}
