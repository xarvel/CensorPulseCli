...
func (panickyListener) OnProgress(int, int, string, string, string, string) { panic("embedder threw") }
func (panickyListener) OnLog(string, string)                                { panic("embedder threw") }

type panickyResolver struct{}

func (panickyResolver) Lookup(string) (string, error) { panic("embedder threw") }

// An exception thrown by the embedder's callback arrives as a Go panic, and
// a Go panic under gomobile kills the app: nothing may cross the binding.
func TestPanicsDoNotCrossTheBinding(t *testing.T) {
	w := &logWriter{l: panickyListener{}}
	if _, err := w.Write([]byte("level=INFO msg=x\n")); err != nil {
		t.Fatalf("logWriter: %v", err)
	}
	quiet(func() { panickyListener{}.OnProgress(1, 2, "t", "g", "v", "ok") })

	if _, err := lookupVia(context.Background(), panickyResolver{}, "example.org"); err == nil {
		t.Fatal("a panicking resolver must surface as an error")
	}
	...
	if err == nil || !strings.Contains(err.Error(), "classifier bug") {
